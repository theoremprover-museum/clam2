/*
 * @(#)$Id: advice.pl,v 1.5 2008/05/21 14:14:14 smaill Exp $
 *
 * $Log: advice.pl,v $
 * Revision 1.5  2008/05/21 14:14:14  smaill
 * update for sicstus4
 *
 * Revision 1.4  1997/01/14 10:45:15  img
 * Generalized conditional for multifile declaration.
 *
 * Revision 1.3  1995/05/17 02:18:54  img
 * 	* Conditional multifile declaration (for Quintus).
 *
 * Revision 1.2  1995/03/01  04:15:15  img
...
file_version('$Id: advice.pl,v 1.5 2008/05/21 14:14:14 smaill Exp $').
...
%   The Goal may have some of its arguments filled in, that doesnt 
...
advised(Goal, Skel, Call) :-
	...
	predicate_property(Skel,(dynamic)),
...
%   are able to put the advice "on the Goals property list" by
%   using recorda/z.
...
advise(_, Skel) :-				% has clauses?
	...
	predicate_property(Skel,(dynamic)),
	!,
	concat_atoms('a$', Functor, Afunctor),
	Skel =.. [Functor |Args],
	Call =.. [Afunctor|Args],
...
advise(Goal, _) :-
	display('! You have no (dynamic) clauses for '),
	display(Goal),
	ttynl,
	fail.
...
pa_explicit(Functor/Arity) :-
	...
	(   clause(Head, 'a$call'(Head,_)), !, nl,
		...
		write(:-(advise(Head,Port,Action))), put_code(46), nl,
		fail
	    ;   true
	    )
...
concat_atoms(Atom1,Atom2,Atom12) :-
	name(Atom1,Chars1),
	name(Atom2,Chars2),
