/*
 * @(#)$Id: writef.pl,v 1.6 2008/05/21 14:14:15 smaill Exp $
 *
 * $Log: writef.pl,v $
 * Revision 1.6  2008/05/21 14:14:15  smaill
 * update for sicstus4
 *
 * Revision 1.5  1997/01/14 10:45:46  img
 * Generalized conditional for multifile declaration.
 *
 * Revision 1.4  1996/07/09 14:57:50  img
 * Spurious assertion removed.
 *
 * Revision 1.3  1995/05/17  02:19:26  img
 * 	* Conditional multifile declaration (for Quintus).
 *
 * Revision 1.2  1995/03/01  04:16:16  img
...
file_version('$Id: writef.pl,v 1.6 2008/05/21 14:14:15 smaill Exp $').
...
prexpr(Term, Nin, Nout, Elements, Z) :-
        ...
        put_char('('), prexpr(A, Nin, Nmid, Elements, Rest),
        put_char(' '), put_code(Conn),
        put_char(' '), prexpr(B, Nmid, Nout, Rest, Z),
        put_char(')').
prexpr(Term, Nin, Nout, [Term|Z], Z) :-
        Nout is Nin+1,
        put_char('X'), write(Nin).
...
wf_char(110, 10).               %  Newline   --UNIX/NIP hack: 31 changed to 10
wf_char(114, 13).               %  Return
wf_char(116,  9).               %  Tab



getcode(Char,Inp,Rest) :-
        getdigits(3, Digits, Inp, Rest), !,
        Digits \== [],
        name(Char, Digits),
        Char < 128.

getdigits(Limit, [Digit|Digits], Inp, Rest) :-
        Limit > 0,
        Inp = [Digit|InpTail],
        "0" =< Digit, 
        Digit =< "9",
        Fewer is Limit-1, !,
        getdigits(Fewer,  Digits, InpTail, Rest).
getdigits(_, [], Inp, Inp).
...
praggl(L, N, F, Term) :-
        arg(L, Term, Arg),
        print(Arg),
        put_code(32), write(F), put_code(32),
        M is L+1, !,
        praggl(M, N, F, Term).
...
getpad(Size, Just, Inp, Rest) :-
        getdigits(3, Digits, Inp, InpTail),
        name(Size, Digits),
        InpTail =[Char|Rest],
        getpad(Char, Just).
